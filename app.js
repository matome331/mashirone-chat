...
(function () {
    ...
    // 読み仮名パターンの正規表現
    // パターン1: 英語 + (ひらがな/カタカナ) → Erai(えらい), CEO(シーイーオー) 等
    // パターン2: 漢字/カタカナ + (ひらがな必須) → 卒倒(そっとう) 等。骨(ボキッ) は除外
    const READING_RE_TEST = /(?:[A-Za-zａ-ｚＡ-Ｚ]+[（(][ぁ-ゖァ-ヶー]+[）)]|[A-Za-zａ-ｚＡ-Ｚ\u4e00-\u9fffァ-ヶ]+[（(](?=[ぁ-ゖァ-ヶー]*[ぁ-ゖ])[ぁ-ゖァ-ヶー]+[）)])/;
    const READING_RE_HIGHLIGHT = /(?:[A-Za-zａ-ｚＡ-Ｚ]+[（(][ぁ-ゖァ-ヶー]+[）)]|[A-Za-zａ-ｚＡ-Ｚ\u4e00-\u9fffァ-ヶ]+[（(](?=[ぁ-ゖァ-ヶー]*[ぁ-ゖ])[ぁ-ゖァ-ヶー]+[）)])/g;
